...
class c_products extends SAdmin{
	...
	private function GetSelect($selectedid =0){
		$sReturn = '<select><option value="0">-- 请选择 --</option>';
		$categories = $this->db->getAll("SELECT  p_id, name, name_en, counts  FROM " . TABLE_PREFIX . "pcat ORDER BY sort");
	...
	private function GetOptions($categories, $selectedid = 0, $p_id = 0, $sublevelmarker = ''){
		...
		foreach($categories as $value){
			if($p_id == $value['p_id']){
				$sReturn = '<option '. Iif(!$p_id, 'style="color:#cc4911;font-weight:bold;"') .  '" '  . '>' . $sublevelmarker . $value['name'] . '-' . $value['name_en'] . ' (' . $value['counts'] . ')</option>';
...
//            $image_size_en = @getimagesize($uploaddir.$file_path_en.$imagename_en);

			CreateImageFile($uploaddir.$file_path.$imagename, $uploaddir.$file_path. $imagename . '_s.jpg', $this->config['siteSmall']);
//            CreateImageFile($uploaddir.$file_path_en.$imagename_en, $uploaddir.$file_path_en. $imagename_en . '_s.jpg', $this->config['siteSmall']);
...
//	}
	...
	private function DeleteProductById($pro_id) {
		$product = $this->db->getOne("SELECT pro_id,  path, filename, filename_en FROM " . TABLE_PREFIX . "product WHERE pro_id = '$pro_id'");
...
//		$this->db->exe("UPDATE " . TABLE_PREFIX . "pcat SET counts = (counts-1) WHERE cat_id = '$product[cat_id]'");
        ...
        $this->UnlinkImage($product['path'], $product['filename_en']); //删除主图片

		$this->DeleteGroupImage($pro_id); //删除组图片
	}

	//按轮播图pro_id 或组图片g_id 删除其图片
	...
	//$search 查询的关键字 $type时间条件 大于小于什么的  $time时间 $cat_id轮播图分类的id
...
//		}
		...
		if($search){
			$Where .= Iif($Where, " AND ") . " (title like '%$search%' OR keywords like '%$search%' OR keywords_en like '%$search%') ";
		}
	...
	public function save(){
        ...
        $pro_name = ForceStringFrom('pro_name');
...
//        $pro_path_en = ForceStringFrom('pro_path_en');
        $pro_filename_en = ForceStringFrom('pro_filename_en');

        $type = ForceStringFrom('type');
        $shop_name = ForceStringFrom('shop_name');
        $shop_name_en = ForceStringFrom('shop_name_en');
        $address = ForceStringFrom('address');
        $address_en = ForceStringFrom('address_en');
        $linkman = ForceStringFrom('linkman');
        $linkman_en = ForceStringFrom('linkman_en');
        $link = ForceStringFrom('link');
		
		$deletethisproduct     = ForceIntFrom('deletethisproduct');

		if($deletethisproduct AND $pro_id){//删除轮播图
			$this->db->exe("DELETE FROM " . TABLE_PREFIX ."product where pro_id='$pro_id'");
//			$this->db->exe("UPDATE " . TABLE_PREFIX . "pcat SET counts = (counts-1) WHERE cat_id = '$oldcat_id'");
            ...
            $this->UnlinkImage($pro_path, $pro_filename_en); //删除多个图片
        ...
        $imagefile_en         = $_FILES['imagefile_en'];
		...
		if(isset($errors))	Error($errors, Iif($pro_id, '编辑轮播图错误', '添加轮播图错误'));

		if($pro_id){//编辑轮播图
			$filesize = $imagefile['size'];
            $filesize_en = $imagefile_en['size'];

			if(($filesize > 0) OR ($filesize_en > 0)){//有主图片文件上传时
//				if(!in_array($imagefile['type'], $valid_image_types) AND !in_array($imagefile_en['type'], $valid_image_types)){
//					$errors[] = '无效的图片文件类型!';
//				}elseif (!IsUploadedFile($imagefile['tmp_name']) || !($imagefile['tmp_name'] != 'none' && $imagefile['tmp_name'] && $imagefile['name']) AND !IsUploadedFile($imagefile_en['tmp_name']) || !($imagefile_en['tmp_name'] != 'none' && $imagefile_en['tmp_name'] && $imagefile_en['name'])){
//					$errors[] ='上传的文件无效!';
//				}


				if(isset($errors)) Error($errors, '编辑轮播图错误');

				$file_path = DisplayDate(time(), 'Y/md');
//                $file_path_en = DisplayDate(time(), 'Y/md');
				$imagename = md5(uniqid(COOKIE_KEY.time()));
                $imagename_en = md5(uniqid(COOKIE_KEY.time())).'_en';
//                $this->UploadImage($imagefile, $file_path, $imagename);
//                $this->UploadImage($imagefile_en, $file_path, $imagename_en);
//                $this->UploadImage($imagefile, $file_path, $imagename);
//                $this->UploadImage($imagefile_en, $file_path, $imagename_en);

                $imagearr = $this->db->getOne("SELECT filename,filename_en FROM "  . TABLE_PREFIX . "product WHERE pro_id = '$pro_id'");
                if($imagefile['name']!='' AND $imagefile_en['name']!=''){
                    $this->UploadImage($imagefile, $file_path, $imagename);
                    $this->UploadImage($imagefile_en, $file_path, $imagename_en);
//
                    $this->db->exe("UPDATE " . TABLE_PREFIX . "product SET 
                        ...
                        sort= '$sort',
                        is_show= '$is_show',
                        " . Iif($filesize AND $filesize_en AND $file_path AND $imagename  AND  $imagename_en, "path = '$file_path', filename = '$imagename', filename_en = '$imagename_en',") . "
                        title = '$title',
                        title_en = '$title_en',
                        keywords = '$keywords',
                ...
                if($imagefile['name']!='' AND $imagefile_en['name']==''){
                    $this->UploadImage($imagefile, $file_path, $imagename);
                    if($imagearr['filename'] == ''){
                        $this->db->exe("INSERT INTO " . TABLE_PREFIX . "product ( pro_name,is_show,  userid, username, path, filename, title, keywords, keywords_en, clicks, created) VALUES ('$pro_name',  '$is_show',  '$userid', '$username', '$file_path', '$imagename','$title', '$keywords', '$keywords_en', '0', '$time', '$address', '$address_en', '$linkman', '$linkman_en', '$link') ");
                    }else {
                        $this->db->exe("UPDATE " . TABLE_PREFIX . "product SET 
                            pro_name =  '$pro_name',
                            sort= '$sort',
...
//                getOne("SELECT * FROM " . TABLE_PREFIX . "product WHERE pro_id = '$pro_id'");
//                print_r($imagefile);
//                print_r($imagefile_en);exit;


                if($imagefile['name']=='' AND $imagefile_en['name']!=''){
                    $this->UploadImage($imagefile_en, $file_path, $imagename_en);
                    if($imagearr['filename_en'] == ''){
                        $this->db->exe("INSERT INTO " . TABLE_PREFIX . "product ( pro_name, is_show, userid, username, path, filename_en, title_en, keywords, keywords_en, clicks, created, type, address, address_en, linkman, linkman_en, linkman) VALUES (  '$pro_name','$is_show',  '$userid', '$username', '$file_path', '$imagename_en','$title_en', '$keywords', '$keywords_en', '0', '$time', '$type', '$address', '$address_en', '$linkman', '$linkman_en', '$link') ");
                    }else{
                        $this->db->exe("UPDATE " . TABLE_PREFIX . "product SET
                            pro_name =  '$pro_name',
        		         	sort= '$sort',
                            is_show= '$is_show',
                            " . Iif($filesize_en AND $file_path  AND $imagename_en, "path = '$file_path', filename_en = '$imagename_en',") . "
            		       	title = '$title',
            		       	title_en = '$title_en',
            		       	keywords = '$keywords',
            		       	keywords_en = '$keywords_en',
...
//				if(!$this->UploadImage($imagefile, $file_path, $imagename) AND !$this->UploadImage($imagefile_en, $file_path, $imagename_en)){
//					Error('处理轮播图图片发生错误!', '编辑轮播图错误');
//				}

//                if(!$this->UploadImage($imagefile_en, $file_path, $imagename_en) OR  !$this->UploadImage($imagefile_en, $file_path, $imagename_en)){
//                    Error('处理轮播图图片发生错误!', '编辑轮播图错误');
//                }
			}else{
                $this->db->exe("UPDATE " . TABLE_PREFIX . "product SET 
...
//			is_show= '$is_show',
//			" . Iif($filesize AND $file_path AND $imagename, "path = '$file_path', filename = '$imagename',") . "
//			" . Iif($filesize AND $file_path AND $imagename_en, "path = '$file_path', filename_en = '$imagename_en',") . "
//			title = '$title',
//			title_en = '$title_en',
//			keywords = '$keywords',
//			keywords_en = '$keywords_en'
...
//			}
            ...
            if($filesize AND $file_path AND $imagename_en){
                $this->UnlinkImage($pro_path, $pro_filename_en);
            }
			...
			foreach($uploaded_images AS $file){
                ...
                $imagename_en = md5(uniqid(COOKIE_KEY.microtime()).'_en');

				if($this->SaveGroupImage($file, $file_path, $imagename)){
					$this->db->exe("INSERT INTO " . TABLE_PREFIX . "gimage (pro_id, pro_name,path, filename) VALUES ('$pro_id','$pro_name', '$file_path', '$imagename') ");
				}
			}
			...
			$filesize = $imagefile['size'];
            $filesize_en = $imagefile_en['size'];
			if($filesize == 0 AND $filesize_en=0)	{
				$errors[] = '未选择图片文件, 或文件大小超过了服务器PHP环境允许上传的文件大小: '.ini_get('upload_max_filesize');
			}elseif(!in_array($imagefile['type'], $valid_image_types) AND !in_array($imagefile_en['type'], $valid_image_types)){
				$errors[] = '无效的图片文件类型!';
			}elseif (!IsUploadedFile($imagefile['tmp_name']) || !($imagefile['tmp_name'] != 'none' && $imagefile['tmp_name'] && $imagefile['name']) AND !IsUploadedFile($imagefile_en['tmp_name']) || !($imagefile_en['tmp_name'] != 'none' && $imagefile_en['tmp_name'] && $imagefile_en['name'])){
				$errors[] ='上传的文件无效!';
			}

			if(isset($errors)) Error($errors, '添加轮播图错误');
            ...
            if($imagefile['name']!='' AND $imagefile_en['name']!=''){
                $this->UploadImage($imagefile, $file_path, $imagename);
                $this->UploadImage($imagefile_en, $file_path, $imagename_en);
                $this->db->exe("INSERT INTO " . TABLE_PREFIX . "product ( pro_name,is_show, userid, username, path, filename, filename_en, title, title_en, keywords, keywords_en, clicks, created, type, shop_name, shop_name_en, address, address_en, linkman, linkman_en, link) VALUES ('$pro_name', '$is_show', '$userid', '$username', '$file_path', '$imagename','$imagename_en','$title','$title_en', '$keywords', '$keywords_en', '0', '$time', '$type', '$shop_name', '$shop_name_en', '$address', '$address_en', '$linkman', '$linkman_en', '$link') ");

            }
            if($imagefile['name']!='' AND $imagefile_en['name']==''){
                $this->UploadImage($imagefile, $file_path, $imagename);
                $this->db->exe("INSERT INTO " . TABLE_PREFIX . "product (pro_name, is_show,  userid, username, path, filename, title, keywords, keywords_en, clicks, created, type, shop_name, shop_name_en, address, address_en, linkman, linkman_en, link) VALUES ( '$pro_name','$is_show',   '$userid', '$username', '$file_path', '$imagename','$title', '$keywords', '$keywords_en', '0', '$time', '$type', '$shop_name', '$shop_name_en', '$address', '$address_en', '$linkman', '$linkman_en', '$link') ");
            }
            if($imagefile['name']=='' AND $imagefile_en['name']!=''){
                $this->UploadImage($imagefile_en, $file_path, $imagename_en);
                $this->db->exe("INSERT INTO " . TABLE_PREFIX . "product (pro_name, is_show,  userid, username, path, filename_en, title_en, keywords, keywords_en, clicks, created, type, shop_name, shop_name_en, address, address_en, linkman, linkman_en, link) VALUES ( '$pro_name','$is_show',  '$userid', '$username', '$file_path', '$imagename_en','$title_en', '$keywords', '$keywords_en', '0', '$time', '$type', '$shop_name', '$shop_name_en', '$address', '$address_en', '$linkman', '$linkman_en', '$link') ");
...
//            if($imagefile AND !$imagefile_en){
//                $this->UploadImage($imagefile, $file_path, $imagename);
//                $this->UploadImage($imagefile_en, $file_path, $imagename_en);
//                $this->db->exe("INSERT INTO " . TABLE_PREFIX . "product ( is_show,  userid, username, path, filename, filename_en,  title,title_en, keywords, keywords_en, clicks, created) VALUES ( '$is_show',  '$userid', '$username', '$file_path', '$imagename','$imagename_en','$title','$title_en', '$keywords', '$keywords_en', '0', '$time') ");
//            }
//
//			if($this->UploadImage($imagefile, $file_path, $imagename)){
//
//				$this->db->exe("INSERT INTO " . TABLE_PREFIX . "product ( is_show,  userid, username, path, filename, title, keywords, keywords_en, clicks, created) VALUES ( '$is_show',  '$userid', '$username', '$file_path', '$imagename',   '$title', '$keywords', '$keywords_en', '0', '$time') ");
//
//				$lastid = $this->db->insert_id;
//				$this->db->exe("UPDATE " . TABLE_PREFIX . "product SET sort = '$lastid' WHERE pro_id = '$lastid'");
//			}else{
//				Error('处理轮播图图片发生错误!', '添加轮播图错误');
//			}
//            if($this->UploadImage($imagefile_en, $file_path, $imagename_en)){
//
//                $this->db->exe("INSERT INTO " . TABLE_PREFIX . "product ( is_show,  userid, username, path, filename_en, title_en, keywords, keywords_en, clicks, created) VALUES ( '$is_show',  '$userid', '$username', '$file_path', '$imagename_en',   '$title_en', '$keywords', '$keywords_en', '0', '$time') ");
//
//                $lastid = $this->db->insert_id;
//                $this->db->exe("UPDATE " . TABLE_PREFIX . "product SET sort = '$lastid' WHERE pro_id = '$lastid'");
//
...
//		$cat_id = ForceIntFrom('c');   //按分类搜索时所选的分类的id
		...
		if($search OR $time){
			SubMenu('轮播图列表', array(array('添加轮播图', 'products/add'), array('全部轮播图', 'products')));
		}else{
			SubMenu('轮播图列表', array(array('添加轮播图', 'products/add')));
		}

//		$newcategories = array();
//		$getcategories = $this->db->query("SELECT p_id, name, name_en, counts FROM " . TABLE_PREFIX . "pcat ORDER BY sort");
...
//		}
		...
		TableHeader('搜索轮播图');
		TableRow('<center><label>搜索:</label>&nbsp;<input type="text" name="s" size="12" value="' . $search . '">&nbsp;&nbsp;&nbsp;<label>发布时间:</label>&nbsp;<select name="type"><option value="gr" ' . Iif($type == 'gr', 'SELECTED') . '>大于(>)</option><option value="eq" ' . Iif($type == 'eq', 'SELECTED') . '>等于(=)</option><option value="le" ' . Iif($type == 'le', 'SELECTED') . '>小于(<)</option></select> <input type="text" name="t" onClick="WdatePicker()" value="' . $time . '" size="12">&nbsp;&nbsp;&nbsp;<select name="c"><option style="color:red;" value="-1" '  . '>未发布的轮播图</option>' .  '</select>&nbsp;&nbsp;&nbsp;&nbsp;<input type="submit" name="searchproduct" value="搜索轮播图" class="cancel"></center>');
		TableFooter();
		echo '</form>';

		$Where = $this->GetSearchSql($search, $type, $time);
		$title = Iif($Where, '搜索到的轮播图列表', '全部轮播图列表');

		$getproducts = $this->db->query("SELECT pro_id,pro_name,sort,is_show, userid, username, path, filename,filename_en, title,title_en,  clicks, created FROM " . TABLE_PREFIX . "product " . $Where . " ORDER BY is_show ASC, sort DESC LIMIT $start, $NumPerPage");
		...
		TableRow(array('排序', '缩图','名称', '轮播图URL','作者', '状态',  '点击数', '日期',  '<input type="checkbox" id="checkAll" for="deletepro_ids[]"> <label for="checkAll">删除</label>'), 'tr0');

		if($maxrows['value'] < 1){
			TableRow('<center><BR><font class=redb>未搜索到任何轮播图!</font><BR><BR></center>');
		}else{
			while($product = $this->db->fetch($getproducts)){
				...
				TableRow(array('<input type="hidden" name="pro_ids[]" value="'.$product['pro_id'].'" /><input type="text" name="sorts[]" value="' . $product['sort'] . '" size="4" />',
					'<a href="'.BURL('products/edit?pro_id='.$product['pro_id']).'"><img src="'. GetImageURL($product['path'], $product['filename']).'" width="40""></a>',
                    '<a href="'.BURL('products/edit?pro_id='.$product['pro_id']).'" title=" '.$product['pro_name'].'">' . $product['pro_name'] . '</a>',
...
//					Iif($product['is_show'], '<a href="'.URL('products?id=' . $product['pro_id']).'" target="_blank"><img src="' . SYSDIR . 'public/admin/images/view.gif"></a>', '<img src="' . SYSDIR . 'public/admin/images/disview.gif">'),
			...
			if($totalpages > 1){
				TableRow(GetPageList(BURL('products'), $totalpages, $page, 10, 's', urlencode($search), 'c',  't', $time, 'type', $type));
			}
		...
		echo '<div class="submit"><input type="submit" name="updateproducts" value="保存更新" class="save"><input type="submit" name="deleteproducts" onclick="'.Confirm('确定删除所选轮播图吗?<br><br><span class=red>注: 所选轮播图的全部信息将被永久删除!</span>', 'form').'" value="删除轮播图" class="cancel"></div></form>';
	}
	...
	public function add(){
		...
		if($pro_id){  //编辑时的
			SubMenu('轮播图管理', array(array('添加轮播图', 'products/add'),array('编辑轮播图', 'products/edit?pro_id='.$pro_id, 1),array('轮播图列表', 'products')));
			...
			SubMenu('轮播图管理', array(array('添加轮播图', 'products/add', 1),array('轮播图列表', 'products')));

			$product = array( 'is_show' => 1);
		}
		...
		if($pro_id){
			TableHeader('编辑轮播图: <span class=note>' . Iif($product['title'], $product['title'], '未命名') . '</span>');
		}else{
			TableHeader('添加轮播图');
		}
...
//		}
		...
		if($pro_id){
            TableRow(array('<b>' . Iif($product['is_show'], '浏览轮播图', '<font class=red>待审轮播图</font>') . ':</b>', '<input type="hidden" name="pro_path" value="' . $product['path'] . '" /><input type="hidden" name="pro_filename" value="' . $product['filename'] . '" /><img src="'. GetImageURL($product['path'], $product['filename']).'" align="top" style="padding-right:26px;float:left;"">'));
		}

		TableRow(array('<B>'. Iif($pro_id, '重新上传轮播图(<span class=blue>中文</span>)', '上传轮播图(<span class=blue>中文</span>)') . ':</B>', '<input name="imagefile" type="file" size="40" />&nbsp;&nbsp;<span class=light>注: <span class=note>仅允许上传JPG类型的图片文件</span>.</span>'));
        TableRow(array('<B>'. Iif($pro_id, '重新上传轮播图(<span class=red>英文</span>)', '上传轮播图(<span class=red>英文</span>)') . ':</B>', '<input name="imagefile_en" type="file" size="40" />&nbsp;&nbsp;<span class=light>注: <span class=note>仅允许上传JPG类型的图片文件</span>.</span>'));
		if($pro_id){
			TableRow(array('<B>是否删除?</B>', '<input type="checkbox" name="deletethisproduct" value="1"> <b>是:</b> <font class=redb>慎选!</font> <span class=light>如果选择删除, 此轮播图相关的所有信息将被删除.</span>'));
		}
        TableRow(array('<B>轮播图名称:</B>', '<input type="text" name="pro_name" value="' . $product['pro_name'] . '"  size="50" />'));

        TableRow(array('<B>轮播图URL(<span class=blue>中文</span>):</B>', '<input type="text" name="title" value="' . $product['title'] . '"  size="50" /> '));
		TableRow(array('<B>轮播图URL(<span class=red>英文</span>):</B>', '<input type="text" name="title_en" value="' . $product['title_en'] . '"  size="50" /> '));

//		TableRow(array('<B>轮播图分类:</B>', '<input type="hidden" name="oldcat_id" value="' . $product['cat_id'] . '">'.$this->GetSelect($product['cat_id']) . ' <font class=red>* 必填项</font>'));

		if($pro_id){
			TableRow(array('<B>排序编号:</B>', '<input type="text" name="sort" value="' . $product['sort'] . '"  size="10" /> <span class=light>注: 当前分类中轮播图将按此编号排序.</span>'));
		}

		TableRow(array('<B>是否发布?</B>', '<input type="checkbox" name="is_show" value="1" '.Iif($product['is_show'] == 1, 'CHECKED').'> <b>是:</b> <span class=light>当不发布时, 此轮播图不在前台显示.</span>'));
//		TableRow(array('<B>是否推荐?</B>', '<input type="checkbox" name="is_best" value="1" '.Iif($product['is_best'] == 1, 'CHECKED').'> <b>是:</b> <span class=light>推荐的轮播图显示在重要的位置.</span>'));

//		TableRow(array('<B>轮播图价格(<span class=blue>中文</span>):</B>', '<input type="text" name="price" value="' . $product['price'] . '" size="30" /> <span class=light>注: 可填写价格及单位.</span>'));
//		TableRow(array('<B>轮播图价格(<span class=red>英文</span>):</B>', '<input type="text" name="price_en" value="' . $product['price_en'] . '" size="30" /> <span class=light>注: 同上</span>'));

		TableRow(array('<B>Meta关键字(<span class=blue>中文</span>):</B>', '<input type="text" name="keywords" value="' . $product['keywords'] . '" size="50" /> <span class=light>注: 轮播图的Meta关键字, <span class=note>便于搜索引擎收录, 请用英文逗号隔开</span>.</span>'));
		...
		TableRow(array('<B>类型</B>', '<input type="radio" name="type" value="1" ' . Iif($product['type'] == 1, 'CHECKED') . '> <b>首页轮播图:</b> <input type="radio" name="type" value="2"' . Iif($product['type'] == 2, 'CHECKED') . '> <b>门店轮播图:</b> '));
		
		TableRow(array('<B>门店名称(<span class=blue>中文</span>):</B>', '<input type="text" name="shop_name" value="' . $product['shop_name'] . '" size="50" /> <span class=light>注: 仅当类型为门店轮播图时展示</span>'));
		TableRow(array('<B>门店名称(<span class=red>英文</span>):</B>', '<input type="text" name="shop_name_en" value="' . $product['shop_name_en'] . '" size="50" /> <span class=light>注: 同上</span>'));
		
		TableRow(array('<B>门店地址(<span class=blue>中文</span>):</B>', '<input type="text" name="address" value="' . $product['address'] . '" size="50" /> <span class=light>注: 同上</span>'));
		TableRow(array('<B>门店地址(<span class=red>英文</span>):</B>', '<input type="text" name="address_en" value="' . $product['address_en'] . '" size="50" /> <span class=light>注: 同上</span>'));
		
		TableRow(array('<B>联系人(<span class=blue>中文</span>):</B>', '<input type="text" name="linkman" value="' . $product['linkman'] . '" size="50" /> <span class=light>注: 同上</span>'));
		TableRow(array('<B>联系人(<span class=red>英文</span>):</B>', '<input type="text" name="linkman_en" value="' . $product['linkman_en'] . '" size="50" /> <span class=light>注: 同上</span>'));

		TableRow(array('<B>联系方式:</B>', '<input type="text" name="link" value="' . $product['link'] . '" size="50" /> <span class=light>注: 同上</span>'));
		
//		TableRow(array('<B>轮播图正文:</B><BR><span class=light>轮播图的详细内容.</span>', '
//			<div class="ok_tab">
...
//		</div>'));
		...
		PrintSubmit(Iif($pro_id, '保存更新', '添加轮播图'));
	}
