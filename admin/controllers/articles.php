...
class c_articles extends SAdmin{
	...
	public function add(){
		...
		TableRow(array('<B>文章标题(<span class=blue>中文</span>):</B>', '<input type="text" name="title" value="' . $articles['title'] . '"  size="50" /> '));
		...
		if($a_id && !empty($articles['default_img'])){
		    TableRow(array('<b>' . Iif(!empty($articles['default_img']), '浏览首图', '<font class=red>没有上传主图</font>') . ':</b>', '<input type="hidden" name="pro_path" value="' . $articles['path'] . '" /><input type="hidden" name="pro_filename" value="' . $articles['default_img'] . '" /><img src="'. ($articles['path'] == 'http' ? $articles['default_img'] : GetImageURL($articles['path'], $articles['default_img'], 2)).'" align="top" style="padding-right:26px;float:left;"">'));
		}
		TableRow(array('<B>'. Iif(($a_id && !empty($articles['default_img'])), '重新上传首图(<span class=blue>中文</span>)', '上传首图(<span class=blue>中文</span>)') . ':</B>', '<input name="imagefile" type="file" size="40" />&nbsp;&nbsp;<span class=light>注: <span class=note>仅允许上传JPG类型的图片文件</span>.</span>'));
		if($a_id && !empty($articles['default_img_en'])){
		    TableRow(array('<b>' . Iif(!empty($articles['default_img_en']), '浏览首图', '<font class=red>没有上传主图</font>') . ':</b>', '<input type="hidden" name="pro_path" value="' . $articles['path'] . '" /><input type="hidden" name="pro_filename" value="' . $articles['default_img_en'] . '" /><img src="'. ($articles['path'] == 'http' ? $articles['default_img_en'] : GetImageURL($articles['path'], $articles['default_img_en'], 2)).'" align="top" style="padding-right:26px;float:left;"">'));
		}
		TableRow(array('<B>'. Iif(($a_id && !empty($articles['default_img_en'])), '重新上传首图(<span class=red>英文</span>)', '上传首图(<span class=red>英文</span>)') . ':</B>', '<input name="imagefile_en" type="file" size="40" />&nbsp;&nbsp;<span class=light>注: <span class=note>仅允许上传JPG类型的图片文件</span>.</span>'));		
		if($a_id && (!empty($articles['default_img_en']) || !empty($articles['default_img']))){
			$update_img_title = '重新';
		}
		TableRow(array('<B>确定'.$update_img_title.'上传首图?</B>', '<input type="checkbox" name="is_update_img" value="1" CHECKED> <b>是:</b> <span class=light>当勾选且未上传首图时, 将删除旧首图.</span>'));
		TableRow(array('<B>信息来源:</B>', '<input type="text" name="source" value="' . $articles['source'] . '" size="50" />'));
		TableRow(array('<B>信息来源(<span class=red>英文</span>):</B>', '<input type="text" name="source_en" value="' . $articles['source_en'] . '" size="50" />'));
	...
	public function save(){
		...
		$source_en = ForceStringFrom('source_en');
		$is_update_img = ForceStringFrom('is_update_img');
		...
		if(isset($errors))	Error($errors, Iif($a_id, '编辑文章首图错误', '添加文章首图错误'));

		$file_path = DisplayDate(time(), 'Y/md');
		$filesize = $imagefile['size'];
		$filesize_en = $imagefile_en['size'];
		$imagename = $imagename_en = '';
		if($filesize > 0) {
		    $imagename = md5(uniqid(COOKIE_KEY.time()));
            $this->UploadImage($imagefile, $file_path, $imagename . '_m.jpg');
		}
		if($filesize_en > 0) {
		    $imagename_en = md5(uniqid(COOKIE_KEY.time())).'_en';
            $this->UploadImage($imagefile_en, $file_path, $imagename_en . '_m.jpg');
		}
		

        //重新上传了主图片时删除原有图片文件
        if (!empty($a_id)) {
            $article = $this->db->getOne('SELECT default_img,default_img_en,path FROM ' . TABLE_PREFIX . 'article WHERE a_id = ' . $a_id);
            if ($article['path'] != 'http') {
	            if($filesize AND $file_path AND $imagename){
	                $this->UnlinkImage($article['path'], $article['default_img']);
	            }
	            if($filesize AND $file_path AND $imagename_en){
	                $this->UnlinkImage($article['path'], $article['default_img_en']);
	            }
            }
            if (!$is_update_img) {
            	$file_path = $article['path'];
            	$imagename = $article['default_img'];
            	$imagename_en = $article['default_img_en'];
            }
        }
        // 上传文章首图
		...
		if(isset($errors)){
			...
			if($a_id){
				$this->db->exe("UPDATE " . TABLE_PREFIX . "article SET 
    				...
    				path = '$file_path',
    				default_img = '$imagename',
    				default_img_en = '$imagename_en'
    				WHERE a_id = ".$a_id);
				if($oldcat_id != $cat_id){
				...
				$this->db->exe("INSERT INTO " . TABLE_PREFIX . "article (cat_id, is_show, is_best, userid, username, title, title_en, content, content_en, source, source_en, keywords, keywords_en, clicks, created, path, default_img, default_img_en) VALUES ('$cat_id', '$is_show', '$is_best', '$userid', '$username', '$title', '$title_en', '$content', '$content_en', '$source', '$source_en', '$keywords', '$keywords_en', '0', '$time', '$file_path', '$imagename', '$imagename_en') ");
