...
var logBorrowNameFun = function(value,rows){
	if(value.length > 8){
		return "<a href='/invest/"+rows.uuid+"/detail.html' title="+value+" target='_blank'>"+value.substring(0,8)+"...</a>";
	}else{
		return "<a href='/invest/"+rows.uuid+"/detail.html' title=" + value + " target='_blank'>" + value + "</a>";
	}
}//标题
...
var logBorrowOperateFun = function(value,rows){
	if(rows.status == 0){
		...
		return "<a href='/member_borrow/borrow/repayment.html?borrowId=" + rows.id + "' >还款明细</a>";
	}else{
		return '-';
	}
}//操作
...
function messageOperate(){
	...
	function messageFun(){
		...
		function msgCommon(obj,fun){
			...
			$.ajax({
				url : operateUrl + typeVal + idVal + "&randomTime=" + (new Date()).getTime(),
				type:"post",
